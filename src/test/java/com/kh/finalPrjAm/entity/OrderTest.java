...
@Transactional
class OrderTest {
    @Autowired // 의존성 주입을 받는 autowired!
    OrderRepository orderRepository;
    @Autowired // 어노테이션은 각극 걸어줘야 함
    ItemRepository itemRepository;
    @Autowired
    MemberRepository memberRepository;
    @PersistenceContext // 영속성에 대한 정보를 저장 & 관리
    EntityManager em;
    ...
    public void cascadeTest() {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            // 아직 영속성 컨텍스트에 저장되지 않은 orderItem 엔티티를 order엔티티에 담아 줌
    ...
    public void orphanRemovalTest() {
        ...
        order.getOrderItemList().remove(0); // order 엔티티에서 관리하고 있는 orderItem리스트의 0번째 인덱스 요소를 제거
        em.flush();
